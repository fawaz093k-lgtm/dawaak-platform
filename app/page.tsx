...
export default async function Home() {
  ...
  const dataSignature = products
    .map((p) => `${p.id}:${p.priceFawaz}:${p.bonusFawaz}`)
    .join("|");
    ...
    <main className="min-h-screen p-4 md:p-8">
      ...
      <header className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-[var(--text-secondary)]">مستودع الفواز — بروشور 25</p>
          </div>
        </div>
        <a
          href="/login"
      ...
      <div className="mb-8">
        <div className="relative max-w-2xl mx-auto">
          <input
            type="text"
            placeholder="ابحث عن دواء... مثال: املودبين، اوميغ، فولتاميد"
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-10">
        ...
        <div className="p-4 rounded-xl bg-[var(--card-bg)] border border-[var(--border)] text-center">
          <p className="font-semibold text-accent">مستودع الفواز</p>
        </div>
        <div className="p-4 rounded-xl bg-[var(--card-bg)] border border-[var(--border)] text-center">
          <p className="font-semibold text-accent">بروشور 25</p>
        </div>
      </div>

      <h2 className="text-xl font-bold mb-4">أدوية الفواز</h2>

      {products.length === 0 ? (
        <p className="text-[var(--text-secondary)]">
          جاري التحميل... تأكد أن شيت المنتجات مفتوح للمشاركة العامة.
        </p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {products.slice(0, 60).map((p) => (
            ...
            >
              ...
              <p className="text-xs text-[var(--text-secondary)] mb-3">{p.company}</p>
      ...
      {products.length > 60 && (
        <p className="mt-4 text-center text-[var(--text-secondary)]">
          عرض 60 من أصل {products.length} — ابحث بالبوت لباقي الأصناف
        </p>
      )}
    </main>
  );
